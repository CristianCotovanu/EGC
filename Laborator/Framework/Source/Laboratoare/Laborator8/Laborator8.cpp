...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	int loc_spotLight = glGetUniformLocation(shader->program, "is_spot");
	glUniform1i(loc_spotLight, isSpot);

	int loc_cutoff = glGetUniformLocation(shader->program, "cut_off_angle");
	glUniform1f(loc_cutoff, cutOffAngle);
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_T)) cutOffAngle += deltaTime * 15;
		if (window->KeyHold(GLFW_KEY_G)) cutOffAngle -= deltaTime * 15;

		if (window->KeyHold(GLFW_KEY_UP)) {
			lightRotation = glm::rotate(glm::mat4(1), deltaTime * 1.5f, glm::vec3(1, 0, 0));
			lightDirection = glm::vec3(lightRotation * glm::vec4(lightDirection, 1));
		}
		...
		if (window->KeyHold(GLFW_KEY_RIGHT)) {
			lightRotation = glm::rotate(glm::mat4(1), -deltaTime * 1.5f, glm::vec3(0, 1, 0));
			lightDirection = glm::vec3(lightRotation * glm::vec4(lightDirection, 1));
		}
